...
void setup() {
  ...
  for(int i = 0; i < numCashiers; i++) {
    cashiers.add(new Cashier(false, 1, new PVector(800*(i+1)/(numCashiers+1), 650), 50));
  }
 
  // Track the last time a customer was served to simulate idle time
  lastCustomerTime = millis();
}
...
void draw() {
  ...
  for(int i = 0; i < cashiers.size(); i++) {
    ...
    float idleTime = (millis() - lastCustomerTime) / 1000.0; // convert to seconds
    ...
    if (frameCount % 120 == 0) {  // Simulate customer every 120 frames (~2 seconds)
      lastCustomerTime = millis(); // Reset idle timer
